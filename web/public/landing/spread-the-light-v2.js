...
function render(progress) {
  const ignition = easeOut(clamp((progress - .305) / .17));
  const sunrise = easeOut(clamp((progress - .435) / .18));
  const settles = easeOut(clamp((progress - .53) / .32));
  const mapAppears = easeOut(clamp((progress - .59) / .14));
  const packetAppears = easeOut(clamp((progress - .81) / .14));

  story.classList.toggle("is-daylight", sunrise > .42);
  story.classList.toggle("has-light-copy", sunrise > .08);
  document.body.classList.toggle("is-daylight", sunrise > .42);
  lamp.style.transform =
    `translate3d(-50%, ${mix(0, window.innerHeight * .33, settles).toFixed(1)}px, 0) ` +
    `scale(${mix(1.08, .62, settles).toFixed(3)})`;
  lampLit.style.opacity = ignition.toFixed(3);
  aura.style.opacity = (ignition * (1 - sunrise * .52)).toFixed(3);
  aura.style.transform =
    `translate(-50%, -50%) scale(${(.4 + ignition * 1.25 + sunrise * 1.8).toFixed(3)})`;
  daylight.style.transform = `translate(-50%, -50%) scale(${(sunrise * 7.2).toFixed(3)})`;
  map.style.opacity = mapAppears.toFixed(3);
  map.style.transform =
    `scale(${mix(.95, 1, mapAppears).toFixed(3)}) translateY(${mix(34, 0, mapAppears).toFixed(1)}px)`;
  packet.style.opacity = packetAppears.toFixed(3);
  packet.style.transform =
    `translate3d(0, ${mix(125, 0, packetAppears).toFixed(1)}%, 0) rotate(${mix(-6, -2, packetAppears).toFixed(2)}deg)`;
}
