...
export function PacketGenerator({ center, mapsApiKey }: PacketGeneratorProps) {
  ...
  const selected = result?.proposals[selectedIndex] ?? null;
  ...
  return (
    <div className="territory-page packet-page">
      <header className="territory-header">
        <div>
          <span className="wordmark">Streetlight</span>
          <span className="phase-label">Packet proposals</span>
        </div>
        <a href="/">Back to coverage</a>
      </header>
            ...
            {result && (
              <section className="packet-results">
                ...
                {result.proposals.map((proposal, index) => {
                  ...
                  return (
                    <article
                      className={`packet-card${active ? ' selected' : ''}`}
